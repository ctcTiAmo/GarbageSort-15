...
# 管理员注册跳转
def managerup(request):
    return render(request, 'common/managerup.html')
# 普通用户注册
    ...
    if ret:  # 判断该手机号格式是否正确
        if len(result1) == 1:  # 判断该手机号是否存在(即判断是否注册过)，如果后台存在记录，则返回相应的提示语句
            ...
            if len(result2) == 1:  # 判断该昵称是否有用户已使用
                ...
                return render(request, 'login.html', status=6)
...
def login_judge(request):
    ...
    if len(result) == 1:  # 判断后台是否存在该用户，有则进一步判断密码是否正确
        ...
        if user_password == password:  # 将用户输入的密码和后台密码进行比对，如果密码一致，判断该账号身份
            ...
            return render(request, 'login.html', status=0)  # 密码错误返回到登入界面
    else:  # 如果不存在该用户则返回相应的提示语句
        return render(request, 'login.html', status=0)  # 账号不存在则返回到登入界面
...
# 设置时间
def timeout():
    a1 = (2022, 1, 1, 0, 0, 0, 0, 0, 0)  # 设置开始日期时间元组（2023-01-01 00：00：00）
    a2 = (2023, 5, 25, 23, 59, 59, 0, 0, 0)  # 设置结束日期时间元组（2023-05-25 23：59：59）
